...
class SaleOrderSchool(models.Model):
    ...
    def action_confirm(self):
        ...
        return super(SaleOrderSchool, self).action_confirm()
    ...
    def create_course_registration(self):
        ...
        for order in self:
            for line in order.order_line:
                ...
                if len(course_ids) > 0:
                    self.sudo().env['climbing_gym_school.course_student'].create_or_confirm_course_registration(self, line, course_ids[0])
                # else:
                #     # create Ticket
                #     _ticket_values = {
                #         'company_id': None,
